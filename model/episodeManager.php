...
class episodeManager extends manager
{
    ...
    {
        $bdd = $this->dbConnect();
        $req = $bdd->prepare('SELECT * FROM posts WHERE stat = 1 ORDER BY chapterNumber DESC');
        $req->execute();
    ...
    {
        $bdd = $this->dbConnect();
        $req = $bdd->prepare('SELECT * FROM posts WHERE chapterNumber = ? ');
    ...
    {
        $bdd = $this->dbConnect();
        $req = $bdd->prepare('SELECT * FROM posts WHERE chapterNumber = ? AND stat = 1');
        $req->execute(array($episodeNumber));
        $data = $req->fetch(PDO::FETCH_OBJ);
